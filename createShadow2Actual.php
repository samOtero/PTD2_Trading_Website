...
if ($reason != "savedOutside") {//
	...
	echo $whichDB;
	$query = "SELECT num, lvl, nickname, m1, m2, m3, m4, item, gender, happy FROM trainer_pokemons WHERE trainerID = ? AND uniqueID = ? AND shiny = 0";
	$result = $dbActual->prepare($query);
	$result->bind_param("ii", $id, $pokeID);
	$result->execute();
	$result->store_result();
	$result->bind_result($pokeNum, $pokeLevel, $pokeNickname, $m1, $m2, $m3, $m4, $item, $pokeGender, $pokeHoF);
	...
	if ($reason != "notFound") {///
		...
		if ($reason != "notEnoughCoins") {////
			if ($reason != "error") {/////
				...
				$query = 'UPDATE trainer_pokemons SET shiny = 2, myTag = "n", happy = 0 WHERE uniqueID = ?';
				$result = $dbActual->prepare($query);
      ...
      <td id="main"><div class="block">
          <div class="title">
            <p>Convert to Shadow - <a href="createShadow2.php?<?php echo $urlValidation ?>">Go Back</a></p>
          </div>
          <div class="content">
           ...
           <p>You don't have enough coins to make this conversion. Press back to continue.</p>
           <?php }else if ($reason == "error") { ?>
           <p>Error in the database. <a href="trading.php">Click here to go back.</a></p>
           <?php } else { 
		    $reason = "success";
		   ?>
            <p>Congratulations! You have converted the following pokemon to shadow:</p>
            <?php } ?>
          </div>
        </div>
        <?php 
		if ( $reason == "success") {
			pokeBox2($pokeNickname, $pokeLevel, 2, $m1, $m2, $m3, $m4, $pokeNum, '&nbsp;', $pokeGender, $item, 0);
		} ?>
        </td>
    </tr>
  </table>
