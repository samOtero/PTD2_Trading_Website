...
<div id="content">
	...
	<table id="content_table">
		<tr>
			...
			<td id="main">
				<div class="block">
					<div class="title"><p>Breeding Center Setup - <a href="breeding_setup2.php?<?php echo $urlValidation ?>">Go Back</a></p></div>
...
<strong>(Note: Items that increase shadow/shiny chances will be consumed in the breeding process.)</strong></p>
						...
						if ($result->affected_rows) {
							echo '<p>Stable Status: Full! You cannot start breeding a new egg at this time. Go back.</p>';
							$moveOn = true;
						}
						$result->free_result();
						$result->close();
						$db2->close();
                        if ($moveOn == false) {
							$femaleID = $_REQUEST['pokeID'];
							$dbActual = get_PTD2_Pokemon_Database($whichDB);
							$query = "SELECT num, lvl, shiny, originalOwner, uniqueID, nickname, myTag, m1, m2, m3, m4, gender, item, happy FROM trainer_pokemons WHERE trainerID = ? AND whichProfile = ? AND ((gender = 2 ".get_Legal_Query().") OR num = 132) AND uniqueID = ?";
							...
							if ($hmp == 0) {
								...
								echo '<div class="block"><div class="content"><p><strong>Second Step:</strong> Choose the second Pokemon for the breeding. This Pokemon is often Male.</p></div></div>';
								$query = "SELECT num, lvl, shiny, originalOwner, uniqueID, nickname, myTag, m1, m2, m3, m4, gender, item, happy FROM trainer_pokemons WHERE trainerID = ? AND whichProfile = ? AND ".$genderQuery." ".get_Egg_Group_Query($pokeNum)." ORDER BY num, lvl";
								//echo $query;
								$result = $dbActual->prepare($query);
								...
								$result->bind_result($pokeNum, $pokeLevel,$pokeShiny, $originalOwner, $pokeID, $pokeNickname, $myTag, $move1, $move2, $move3, $move4, $pokeGender, $pokeItem, $pokeHoF);
								if ($hmp == 0) {
									echo '<div class="block"><div class="content"><p>You have no Pokemon that is compatible with your first choice. Go back and catch or trade for some Pokemon.</p></div></div>';	
								}else{
									for ($i=1; $i<=$hmp; $i++) {
										$result->fetch();
										$isHacked = "";
										...
										$pokeNickname = stripslashes($pokeNickname).$isHacked;
										$extra = '<a href="breeding_setup_final2.php?'.$urlValidation.'&pokeID='.$pokeID.'&femaleID='.$femaleID.'">Pick Me as the Second Pokemon</a>';
										pokeBox2($pokeNickname, $pokeLevel, $pokeShiny, $move1, $move2, $move3, $move4, $pokeNum, $extra, $pokeGender, $pokeItem, $pokeHoF);
									}
								}
							}
