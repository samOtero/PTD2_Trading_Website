	...
	<table id="content_table">
		<tr>
			...
			<td id="main">
				...
				<div class="block">
					<div class="title"><p>Transfer to PTD3 Completed - <a href="transferTo3.php?<?= $urlValidation?>">Go Back</a></p></div>
...
if ($pokeGenderRequest == 1 || $pokeGenderRequest == 2) {
 ...
 <?php 
	...
	if ($result->affected_rows) {
		...
		$extra1 = 0;
		$extra2 = 0;
		$query2 = "INSERT INTO ptdtrad_ptd2_trading.ptd3_pickup_pokes (num, lvl, exp, shiny, nickname, m1, m2, m3, m4, item, originalTrainer, currentTrainer, myTag, gender, extra1, extra2) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		$result2 = $db_New->prepare($query2);
		$result2->bind_param("iiiisiiiiiiisiii", $pokeNum, $pokeLevel, $pokeExp, $pokeShiny, $pokeNickname, $m1, $m2, $m3, $m4, $item, $originalOwner, $id, $myTag, $pokeGender, $extra1, $extra2);
		$result2->execute();
		$result2->close();
		$db_New->close();
        ...
        <div class="block">
        <div class="content">
		<p>Pokemon transfered to your PTD3 Pickup area! <a href="transferTo3.php?<?= $urlValidation?>">Click here to go back.</a></p>
	</div>
 </div>
        <?php
		/////////////////////////////////////////////////////////////
