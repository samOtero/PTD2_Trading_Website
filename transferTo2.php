...
<div id="content">
	...
	<table id="content_table">
		<tr>
			...
			<td id="main">
				<div class="block">
					<div class="title"><p>Transfer to PTD2 - <a href="checkPokemon.php?<?php echo $urlValidation ?>">Go Back</a></p></div>
                ...
                <?php }else { ?>
					<div class="content">
						<p>Here is a list of your pokemon from this profile that are eligible to be transfered to PTD2, click on the gender you want it to be to transfer it to the PTD2 Pickup area.</p>
						<p>NOTE: This will remove the pokemon from your profile. <strong>YOU CANNOT TRANSFER THEM BACK!</strong> Also their moves will reset when transfered to PTD2 to the moves they would get at level 1 of their stage 1 evolution.</p>
					</div>
				</div>
                <?php 
$dbActual = get_Pokemon_Database($whichDB, $db);
	...
	for ($i=1; $i<=$hmp; $i++) {
		...
		//}
		$extra = 'Transfer <a href="transferTo2actual.php?'.$urlValidation.'&pokeGender=1&pokeID='.$pokeID.'">Male</a> | <a href="transferTo2actual.php?'.$urlValidation.'&pokeGender=2&pokeID='.$pokeID.'">Female</a>';
		if (($pokeNum >= 29 && $pokeNum <= 31) || $pokeNum == 124 || $pokeNum == 113 || $pokeNum == 115) {
			$extra = 'Transfer <a href="transferTo2actual.php?'.$urlValidation.'&pokeGender=2&pokeID='.$pokeID.'">Female</a>';
		}else if (($pokeNum >= 32 && $pokeNum <= 34) || ($pokeNum >= 106 && $pokeNum <= 107) || $pokeNum == 128) {
			$extra = 'Transfer <a href="transferTo2actual.php?'.$urlValidation.'&pokeGender=1&pokeID='.$pokeID.'">Male</a>';
			...
			$extra = 'Transfer <a href="transferTo2actual.php?'.$urlValidation.'&pokeGender=-1&pokeID='.$pokeID.'">Genderless</a>';
		}
