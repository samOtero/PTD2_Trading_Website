...
if ($reason != "savedOutside") {//
	...
	$query = "SELECT num FROM trainer_pokemons WHERE trainerID = ? AND myTag = 'h'";
	$result = $dbActual->prepare($query);
	$result->bind_param("i", $id);
	$result->execute();
	$result->store_result();
	$result->bind_result($pokeNum);
	if ($result->affected_rows < 10) {
		$reason = "notFound";
	}else{
		$result->fetch();
	}
	...
	if ($reason != "notFound") {///
		...
		if ($reason != "notEnoughCoins") {////
			if ($reason != "error") {/////
				...
				$query = 'UPDATE trainer_pokemons SET myTag = "n" WHERE trainerID = ? AND myTag = "h"';
				$result = $dbActual->prepare($query);
				$result->bind_param("i", $id);
      ...
      <td id="main"><div class="block">
          ...
          <div class="content">
           ...
           <p>You don't own enough pokemon with a hacked tag to make this worth it. Press back to continue.</p>
           ...
           <?php } else { 
		    $reason = "success";
		   ?>
            <p>Congratulations! You have removed the hack tag on all your pokemon!</p>
            <?php } ?>
          </div>
        </div>
        </td>
