	...
	if ($reason != "notFound") {///
		...
		if ($reason != "notEnoughCoins") {////
			if ($reason != "error") {/////
				...
				$query = 'UPDATE trainer_pokemons SET shiny = 1, myTag = "n", happy = 0 WHERE uniqueID = ?';
				$result = $dbActual->prepare($query);
      ...
      <td id="main"><div class="block">
          <div class="title">
            <p>Convert to Shiny - <a href="createShiny2.php?<?php echo $urlValidation ?>">Go Back</a></p>
          </div>
          <div class="content">
           ...
           <?php } else { 
		    $reason = "success";
		   ?>
            <p>Congratulations! You have converted the following pokemon to shiny:</p>
            <?php } ?>
          </div>
        </div>
        <?php 
		if ( $reason == "success") {
			pokeBox2($pokeNickname, $pokeLevel, 1, $m1, $m2, $m3, $m4, $pokeNum, '&nbsp;', $pokeGender, $item, 0);
		} ?>
        </td>
    </tr>
  </table>
